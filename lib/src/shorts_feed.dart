...
class _ShortsFeedState extends State<ShortsFeed> {
  ...
  Future<void> _bootstrap() async {
    ...
    await _ctrl.prewarmAround(0, forward: widget.preloadWindow.fwd, backward: widget.preloadWindow.back);
    await _ctrl.onActive(0);
    // прогрев превью
    _ensureThumb(0);
    for (int i=1;i<=widget.preloadWindow.fwd;i++) { _ensureThumb(i); }
    setState(() {});
  }
  ...
  void _onScroll() {
    ...
    final dtMs = _lastTs == null ? 0.0 : max(1.0, now.difference(_lastTs!).inMilliseconds.toDouble());
    ...
    _prewarmDirection(dir, target, _speedPxPerMs >= widget.scrollPrewarm.fastSwipeSpeed);
  }
  ...
  Future<void> _changePage(int i) async {
    ...
    await _ctrl.prewarmAround(i, forward: widget.preloadWindow.fwd, backward: widget.preloadWindow.back);
    _ensureThumb(i);
        ...
        itemBuilder: (ctx, i) {
          final overlay = widget.overlayBuilder?.call(
            ...
            FeedOverlayState(
              ...
              direction: (( _pageCtrl.page ?? _current.toDouble()) - _current).sign.toInt(),
...
class _FeedItemState extends State<_FeedItem> with SingleTickerProviderStateMixin {
